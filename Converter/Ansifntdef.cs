...
namespace MediaSupport
{
    public class Ansifntdef
    {
        ...
        private void BuildFntBits()
        {
            ...
            for (int y = 0; y <= m_FontImage.Height - 1; y++)
            {
                ...
                for (int x = 0; x <= m_FontImage.Width - 1; x++)
                {
                    ...
                    {
                        ConverterSupport.Convert.SetBit(ref bt, System.Convert.ToByte(btCnt));
                    }
                    if (btCnt == 8)
                    {
        ...
        public Bitmap DrawText(bool bNoCols = false)
        {
            ...
            try
            {
                bm = ConverterSupport.Convert.BitmapToIndexed(new Bitmap(iMaxWidth * m_Fontwidth, iMaxHeight * m_FontHeight), m_Palette);
            }
            catch (Exception ex)
            {
            }
            ...
            for (int a = 0; a <= iMaxHeight - 1; a++)
            {
                ...
                for (int FntCharPos = 1; FntCharPos <= iMaxWidth; FntCharPos++)
                {
                    ...
                    charimg = ConverterSupport.Convert.BitmapToIndexed(new Bitmap(m_Fontwidth, m_FontHeight), m_Palette);
                    ...
                    gr.FillRectangle(new SolidBrush(InternalConstants.AnsiColorsARGB[UseBackColID]), destRect);
        ...
        public Bitmap DrawTextFromString(string stext)
        {
            ...
            bte = ConverterSupport.Convert.StringToByteArray(Join(aworktxt, ""));

            return ConverterSupport.Convert.ByteArrayToBitmap(bte, iMaxWidth, iMaxHeight, bIsSmallFont);
        }
    }
}