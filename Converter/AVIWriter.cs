...
namespace MediaSupport
{
    public class AviWriter
    {
        ...
        public void OpenAVI(string fileName, UInt32 frameRate, int w = 0, int h = 0)
        {
            ...
            int OpeningError = MediaFormats.Avi.AVIFileOpen(ref aviFile, fileName, 4097, 0);
            if (OpeningError != 0)
            {
        ...
        public void AddFrame(Bitmap bmp_)
        {
            ...
            if (countFrames == 0)
            {
                uint bmpDatStride = (UInt32)bmpData.Stride;
            ...
            System.Math.Max(System.Threading.Interlocked.Increment(ref countFrames), countFrames - 1);
        }
        ...
        private void CreateStream()
        {
            ...
            int createResult = MediaFormats.Avi.AVIFileCreateStream(aviFile, ref aviStream, ref strhdr);
            if (createResult != 0)
            {
            ...
            uint bisize = (UInt32)Marshal.SizeOf(bi);
            ...
            int formatResult = MediaFormats.Avi.AVIStreamSetFormat(aviStream, 0, ref bi, Marshal.SizeOf(bi));
            if (formatResult != 0)
            {
